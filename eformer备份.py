...
class Attention4DDownsample(nn.Layer):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        q = self.q(x).flatten(2).reshape([B, self.num_heads, -1, H * W // 4]).transpose([0, 1, 3, 2])
        k = self.k(x).flatten(2).reshape([B, self.num_heads, -1, H * W]).transpose([0, 1, 2, 3])
        v = self.v(x)
        v_local = self.v_local(v)
        v = v.flatten(2).reshape([B, self.num_heads, -1, H * W]).transpose([0, 1, 3, 2])
...
class EfficientFormer(nn.Layer):
    ...
    def __init__(self, layers, 
                 in_channels=3,mode='multi_scale',
                 embed_dims=None,
                 mlp_ratios=4, 
                 downsamples=None,
                 pool_size=3,
    ...
    def forward(self, x):
        ...
        if self.fork_feat:
            if self.mode is not 'multi_scale':
                x = paddle.concat([F.interpolate(feat, 
                                                size=x[0].shape[-2:],
                                                mode='bilinear') for feat in x], axis=1)
